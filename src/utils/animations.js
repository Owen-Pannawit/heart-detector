  ...
  animate(ctx, centerX, centerY, detected, canvasWidth, canvasHeight) {
    ...
    if (detected) {
      ...
      const maxRippleSize =
        ...
        1.5;
      const rippleSize = 20 + progress * maxRippleSize;

      ctx.save();
      ctx.fillStyle = "#ec4899";
      drawHeartPath(ctx, centerX, centerY - rippleSize / 2, rippleSize);
      ctx.fill();
      ctx.restore();
      ...
      if (this.activeRipple) {
        ...
        if (progress >= 1 && !this.isTriggered) {
...
export class BouquetFlower {
  ...
  update() {
    ...
    if (
      this.stemHeight > this.maxStemHeight * 0.3 &&
      this.bloomSize < this.maxBloomSize
    ) {
      this.bloomSize += 2;
    }
  }
  ...
  animate(
    ...
    if (isFist) {
      ...
      if (!this.isInitialized) {
        ...
        this.smoothBottomX = this.lerp(
          this.smoothBottomX,
          targetBottomX,
          smoothFactor,
        );
        this.smoothBottomY = this.lerp(
          this.smoothBottomY,
          targetBottomY,
          smoothFactor,
        );
        this.smoothAngle = this.lerp(
          this.smoothAngle,
          targetAngle,
          smoothFactor,
        );
